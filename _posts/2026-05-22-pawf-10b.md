...
date: 2026-05-22
permalink: /posts/2026-05-22-django-personal-website/
header:
  teaser: "posts/django/pawf10x.png"
...
Example comment syntax: start with `{% raw %}{% comment %}{% endraw %}` and close with `{% raw %}{% endcomment %}{% endraw %}`.

```text
{% raw %}{% comment %} This comment will not appear in the browser {% endcomment %}{% endraw %}
```

We can also pass data from a view to a template with a context dictionary.

Update `about_page_view` on `pages/views.py`:
...
Update `about.html` on `pages/templates/pages/about.html`:
...
In this third Django session, we practiced the core ideas in the local `project3` setup with Django 6:
